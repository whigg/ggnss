...
constexpr std::size_t antenna_full_max_chars{
    ...
    + antenna_radome_max_chars + antenna_serial_max_chars +
    1 /* null-reminating char */
};
...
public:
  ...
  ReceiverAntenna() noexcept { initialize(); }

  /// @brief Constructor from a c-string (model+radome)
  explicit ReceiverAntenna(const char *) noexcept;

  /// @brief Constructor from a string (model+radome)
  explicit ReceiverAntenna(const std::string &) noexcept;

  /// @brief Equality operator (checks antenna type, radome and serial nr).
  bool is_same(const ReceiverAntenna &) const noexcept;
...
private:
  ...
  bool radome_is_empty() const noexcept;
...
public:
  ...
  SatelliteAntenna() noexcept { initialize(); }

  /// @brief Constructor from a c-string containing the antenna type
  explicit SatelliteAntenna(const char *c) noexcept;

  /// @brief Compare satellite antenna types
  bool is_same(const char *) const noexcept;

  /// @brief Set model from a c-string (acts like a constructor)
  inline void set_from_cstr(const char *c) noexcept { copy_from_cstr(c); }

  /// @brief Get the underlying c-string
  const char *__underlying_char__() const noexcept { return __name; }

  /// @brief get the antenna as std::string
  std::string to_string() const noexcept { return std::string(__name); }
