...
public:
  ...
  int set_from_rnx3(std::ifstream &inp) noexcept;

  template <typename T, typename = std::enable_if_t<T::is_of_sec_type>>
  ngpt::datetime<T> toe2date() const {
    switch (this->sys__) {
    case (SATELLITE_SYSTEM::gps):
      return gps_toe2date<T>();
    case (SATELLITE_SYSTEM::glonass):
      return glo_toe2date<T>();
    case (SATELLITE_SYSTEM::galileo):
      return gal_toe2date<T>();
    case (SATELLITE_SYSTEM::beidou):
      return bds_toe2date<T>();
    case (SATELLITE_SYSTEM::sbas):
    case (SATELLITE_SYSTEM::qzss):
    case (SATELLITE_SYSTEM::irnss):
    case (SATELLITE_SYSTEM::mixed):
      std::cerr << "\n[ERROR] NavDataFrame::toe2date() Cannot handle satellite "
                   "system: "
                << satsys_to_char(sys__);
      throw std::runtime_error(
          "ERROR] NavDataFrame::toe2date() Cannot handle satellite system");
    }
    return datetime<T>::min();
  }

  template <typename T, typename = std::enable_if_t<T::is_of_sec_type>>
  int stateNclock(const ngpt::datetime<T> &t, double *state,
                  double &clock) const {
    switch (this->sys__) {
    case (SATELLITE_SYSTEM::gps):
      return gps_stateNclock(t, state, clock);
    case (SATELLITE_SYSTEM::glonass):
      return glo_stateNclock(t, state, clock);
    case (SATELLITE_SYSTEM::galileo):
      return gal_stateNclock(t, state, clock);
    case (SATELLITE_SYSTEM::beidou):
      return bds_stateNclock(t, state, clock);
    case (SATELLITE_SYSTEM::sbas):
    case (SATELLITE_SYSTEM::qzss):
    case (SATELLITE_SYSTEM::irnss):
    case (SATELLITE_SYSTEM::mixed):
      std::cerr << "\n[ERROR] NavDataFrame::stateNclock() Cannot handle "
                   "satellite system: "
                << satsys_to_char(sys__);
      throw std::runtime_error(
          "ERROR] NavDataFrame::stateNclock) Cannot handle satellite system");
    }
    return 100;
  }
  ...
  int sv_health() const {
    switch (this->sys__) {
    case (SATELLITE_SYSTEM::glonass):
      return static_cast<int>(data__[6]);
    case (SATELLITE_SYSTEM::gps):
    case (SATELLITE_SYSTEM::galileo):
    case (SATELLITE_SYSTEM::beidou):
    ...
    case (SATELLITE_SYSTEM::irnss):
    case (SATELLITE_SYSTEM::mixed):
      std::cerr << "\n[ERROR] NavDataFrame::sv_health() Cannot handle "
                   "satellite system: "
                << satsys_to_char(sys__);
      throw std::runtime_error(
          "ERROR] NavDataFrame::sv_clock() Cannot handle satellite system");
    }
    return 100;
  }

  long fit_interval() const {
    switch (this->sys__) {
    case (SATELLITE_SYSTEM::glonass):
      return 15 * 60L;
    case (SATELLITE_SYSTEM::gps):
      return gps_fit_interval();
    case (SATELLITE_SYSTEM::galileo):
    case (SATELLITE_SYSTEM::beidou):
      return 4L * 60 * 60;
    case (SATELLITE_SYSTEM::sbas):
    ...
    case (SATELLITE_SYSTEM::irnss):
    case (SATELLITE_SYSTEM::mixed):
      std::cerr << "\n[ERROR] NavDataFrame::fit_interval() Cannot handle "
                   "satellite system: "
                << satsys_to_char(sys__);
      throw std::runtime_error(
          "ERROR] NavDataFrame::fit_interval() Cannot handle satellite system");
    }
    return 100;
  }
  ...
  template <typename T>
  inline double ref2toe(const ngpt::datetime<T> &t) const noexcept {
    double tsec = t.sec().to_fractional_seconds();
    int mjd_diff =
        t.mjd().as_underlying_type() - toe__.mjd().as_underlying_type();
    if (mjd_diff)
  ...
  template <typename T>
  inline double ref2toc(const ngpt::datetime<T> &t) const noexcept {
    double tsec = t.sec().to_fractional_seconds();
    int mjd_diff =
        t.mjd().as_underlying_type() - toc__.mjd().as_underlying_type();
    if (mjd_diff)
      tsec += 86400e0 * (double)mjd_diff;
    return tsec;
  }
  ...
  template <typename T>
  int gps_stateNclock(ngpt::datetime<T> t, double *state, double &dt) const
      noexcept {
    int status = 0;
    double t_sec = this->ref2toe<T>(t);
    // if ((status=gps_ecef(t_sec, state))) return status;
    if ((status = this->kepler2state<SATELLITE_SYSTEM::gps>(t_sec, state)))
      return status;
  ...
  template <typename T>
  int gal_stateNclock(ngpt::datetime<T> t, double *state, double &dt) const
      noexcept {
    int status = 0;
    double t_sec = this->ref2toe<T>(t);
    // if ((status=gal_ecef(t_sec, state))) return status;
    if ((status = this->kepler2state<SATELLITE_SYSTEM::galileo>(t_sec, state)))
      return status;
    t_sec = this->ref2toc<T>(t);
  ...
  template <typename T>
  int bds_stateNclock(ngpt::datetime<T> t, double *state, double &dt) const
      noexcept {
    int status = 0;
    double t_sec = this->ref2toe<T>(t);
    // if ((status=bds_ecef(t_sec, state))) return status;
    if ((status = this->kepler2state<SATELLITE_SYSTEM::beidou>(t_sec, state)))
      return status;
    t_sec = this->ref2toc<T>(t);
  ...
  template <typename T>
  int glo_stateNclock(ngpt::datetime<T> t, double *state, double &dt) const
      noexcept {
    int status = 0;
    double t_sec = this->ref2toe<T>(t);
    if ((status = glo_ecef(t_sec, state)))
      return status;
    if ((status = glo_clock(t_sec, dt)))
      return status;
    return 0;
  }

  double data(int idx) const noexcept { return data__[idx]; }

  double &data(int idx) noexcept { return data__[idx]; }

  SATELLITE_SYSTEM
  system() const noexcept { return sys__; }

  int prn() const noexcept { return prn__; }

  SATELLITE_SYSTEM &system() noexcept { return sys__; }

  int &prn() noexcept { return prn__; }

  ngpt::datetime<ngpt::seconds> toc() const noexcept { return toc__; }

  template <typename T, typename = std::enable_if_t<T::is_of_sec_type>>
  ngpt::datetime<T> toc() const noexcept {
    return toc__.cast_to<T>();
  }

  template <typename T, typename = std::enable_if_t<T::is_of_sec_type>>
  ngpt::datetime<T> toe() const noexcept {
    return toe__.cast_to<T>();
  }

  void set_toc(ngpt::datetime<ngpt::seconds> d) noexcept { toc__ = d; }
...
private:
  ...
  /// @brief get SV coordinates and velocity (PZ90) refferenced to SV mass
  /// centre
  int glo_ecef(double tb_sod, double *state) const noexcept;
  ...
  /// same position (that is at the same indexes in data__ array). Hence, we
  /// only need one function to turn these elements to SV position. For more
  /// information, see the (now obsolete function gps_ecef, gal_ecef and
  /// bds_ecef).
  ...
  int kepler2state(double t_sec, double *state, double *Ek_ptr = nullptr) const
      noexcept {
    int status = 0;

    constexpr double MI_SYS = ngpt::satellite_system_traits<S>::mi();
    constexpr double OMEGAE_SYS =
        ngpt::satellite_system_traits<S>::omegae_dot();
    ...
    const double n0(
        std::sqrt(MI_SYS / (A * A * A))); //  Computed mean motion (rad/sec)
    ...
    const double duk(data__[9] * sin2F +
                     data__[7] * cos2F); // Argument of Latitude
                                         //+ Correction
    const double drk(data__[4] * sin2F +
                     data__[16] * cos2F); // Radius Correction
    const double dik(data__[14] * sin2F +
                     data__[12] * cos2F); // Inclination Correction

    const double uk(Fk + duk);                   // Corrected Argument
                                                 //+ of Latitude
    const double rk(A * (1e0 - e * cosE) + drk); // Corrected Radius
    const double ik(data__[15] + dik +
                    data__[19] * tk); // Corrected Inclination
    ...
    const double omega_k(data__[13] + (data__[18] - OMEGAE_SYS) * tk -
                         OMEGAE_SYS * data__[11]);
  ...
  int glo_clock(double t_sec, double &dtsv) const noexcept;
  ...
  /// same position (that is at the same indexes in data__ array). Hence, we
  /// only need one function to turn these elements to SV clock correction. For
  /// more information, see the (now obsolete function gps_ecef, gal_ecef and
  /// bds_ecef).
  /// @tparam S         The satellite system of the NavDataFrame; only works
  ...
  /// @return Anything other than 0 denotes an error
  template <SATELLITE_SYSTEM S>
  int sv_clock(double t_sec, double &dt_sv, double *Ein = nullptr) const
      noexcept {
    constexpr double MI_SYS = ngpt::satellite_system_traits<S>::mi();
  ...
  */
  ...
  /// datetime<T> instance.
  /// TimeSystem is GPST
  template <typename T> ngpt::datetime<T> gps_toe2date() const noexcept {
  ...
  template <typename T> ngpt::datetime<T> gal_toe2date() const noexcept {
  ...
  template <typename T> ngpt::datetime<T> bds_toe2date() const noexcept {
  ...
  template <typename T> ngpt::datetime<T> glo_toe2date() const noexcept {
    long sw_toc;
    ...
    if (!health) {
      ...
      /*if (std::abs(test-(double)sd_toe)>1e-15) {
        std::cerr<<"\n[ERROR] ToE incorrect!
      PRN:"<<satsys_to_char(sys__)<<prn__; ngpt::datetime<T>
      tmp(toc__.mjd()-modified_julian_day(offset), ngpt::cast_to<seconds,
      T>(seconds(sd_toe))); std::cerr<<"\n        ToC
      "<<ngpt::strftime_ymd_hms<ngpt::seconds>(toc__); std::cerr<<"\n        ToE
      "<<ngpt::strftime_ymd_hms<T>(tmp)<<" from nav: "<<data__[2];
        std::cerr<<"\nSD: "<<sd_toe<<" Test: "<<test;
      }*/
    } else {
      if (std::abs(test - (double)sd_toe) < 1e-15) {
        std::cerr << "\n[WARNING] NavDataFrame::glo_toe2date() ToE not set "
                     "correctly for unhealhty satellite: "
                  << satsys_to_char(sys__) << prn__;
      }
    }
#endif
...
public:
  ...
  explicit NavigationRnx(const char *);
  ...
  /// @param[out] frame   NavDataFrame read (if found) that matches the
  /// requested
  ...
  ///                     * >0 An error occured
  template <typename T, typename = std::enable_if_t<T::is_of_sec_type>>
  int find_next_valid(const ngpt::datetime<T> &t, pos_type &curpos,
                      NavDataFrame &frame, SATELLITE_SYSTEM sys,
                      int prn) noexcept {
    // std::cout<<"\n$$$[DEBUG] Updating msg for
    // t="<<ngpt::strftime_ymd_hms<ngpt::milliseconds>(t);
#ifdef DEBUG
    if (!__istream.good())
      return 50;
#endif
    ...
    do {
      ...
      if (csys == sys) {
        ...
        if (prn == frame.prn()) {
          ...
          // std::cout<<" found msg with ToC
          // "<<ngpt::strftime_ymd_hms<ngpt::milliseconds>(toc); are we too far
          // ahead from the input date ??
          T limitT(3600L * 4 *
                   T::template sec_factor<long>()); // 4 hour in T units
          // if (toc>t && ngpt::delta_sec(toc, t)>limitT) std::cout<<" t vs ToC
          // too far away!";
          if (toc > t && ngpt::delta_sec(toc, t) > limitT)
            return -1;
          // check SV health
          if (!(frame.sv_health())) {
            ...
            if (sys != SATELLITE_SYSTEM::glonass) {
              ...
              // std::cout<<" msg valid untill
              // "<<ngpt::strftime_ymd_hms<ngpt::milliseconds>(max_t); if
              // (t>=toc && t<max_t) std::cout<<" msg OK!"; else std::cout<<"
              // msg not ok!";
              if (t >= toc && t < max_t)
                return 0;
            } else {
...
private:
  ...
  int clear_stream(int exit_status) noexcept;
