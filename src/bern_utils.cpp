...
ngpt::BernSatellit::BernSatellit(const char *fn)
    : __filename(fn), __istream(fn, std::ios_base::in), __part2(0) {
  if (initialize()) {
    if (__istream.is_open())
      __istream.close();
    throw std::runtime_error(
        "[ERROR] BernSatellit::BernSatellit Failed to read SATELLIT header");
  }
}
...
int ngpt::BernSatellit::get_frequency_channel(
    int svn, const ngpt::datetime<ngpt::seconds> &eph, int &ifrqn, int &prn) {
  ...
  while (!satellite_matched && line_count < max_lines) {
    ...
    if (line[5] == 'M' && line[6] == 'W') {
      ...
      if (csvn == svn) {
        ...
        if (eph >= start && eph < stop) {
          ...
          if (errno == ERANGE) {
            errno = 0;
            // throw std::runtime_error("[ERROR] Failed to resolve svn from
            // SATELLIT file");
            return 3;
          } else {
            return 0;
    ...
    if (std::strlen(line) < 10 || !std::strncmp("PART 3", line, 6))
      return -1;
  }
...
int ngpt::BernSatellit::initialize() noexcept {
  ...
  if (!__istream.good() || line_count >= max_lines)
    return 11;
  // next line is just a series of  '-' chars
  __istream.getline(line, MAX_SATELLIT_CHARS);
  // next two lines are column descriptions (for the lines that follow)
  __istream.getline(line, MAX_SATELLIT_CHARS);
  const char *hln1 = "                                              START TIME "
                     "          END TIME                 SENSOR OFFSETS (M)    "
                     "   SENSOR BORESIGHT VECTOR (U) SENSOR AZIMUTH VECTOR (N)";
  const int hln1_sz = std::strlen(hln1);
  if (std::strncmp(hln1, line, hln1_sz))
    return 12;
  __istream.getline(line, MAX_SATELLIT_CHARS);
  const char *hln2 =
      "PRN  TYPE  SENSOR NAME______SVN  NUMBER  YYYY MM DD HH MM SS  YYYY MM "
      "DD HH MM SS         DX        DY        DZ         X       Y       Z    "
      "      X       Y       Z      ANTEX SENSOR NAME___  IFRQ  SIGNAL "
      "LIST___________------>";
  const int hln2_sz = std::strlen(hln2);
  if (std::strncmp(hln2, line, hln2_sz))
    return 13;
