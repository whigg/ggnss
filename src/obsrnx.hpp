...
public:
  ...
  explicit ObservationRnx(const char *);
  ...
  auto time_of_first_obs() const noexcept { return __epoch_start; }
  ...
  void print_members() const noexcept {
    std::cout << "\nfilename     :" << __filename
              ...
              << "\nApprox. Pos. :" << __approx[0] << ", " << __approx[1]
              << ", " << __approx[2] << "\nEccentricity :" << __eccentricity[0]
              << ", " << __eccentricity[1] << ", " << __eccentricity[2]
              << "\nRcv Clk Off  :" << (__rcv_clk_offs_applied ? "Yes" : "No");
    for (auto const &[key, val] : __obstmap) {
      std::cout << "\n" << satsys_to_char(key);
      for (auto const &o : val)
  ...
  std::map<SATELLITE_SYSTEM, std::vector<vecof_idpair>> set_read_map(
      /*const */ std::map<SATELLITE_SYSTEM, std::vector<GnssObservable>> &inmap,
      bool skip_missing = false) const noexcept;
  ...
  /// @brief Initialize a big enough vector to hold any epoch in current
  /// instance
  std::vector<std::pair<ngpt::Satellite, std::vector<double>>>
  initialize_epoch_vector(
...
private:
  ...
  int __resolve_epoch_304__(const char *cline, ngpt::modified_julian_day &mjd,
  ...
  ///        record line
  int sat_epoch_collect(const std::vector<vecof_idpair> &sysobs, int &prn,
                        std::vector<double> &vals) const noexcept;

  /// @brief Collect
  int collect_epoch(int numsats, int &satscollected,
                    std::map<SATELLITE_SYSTEM, std::vector<vecof_idpair>> &mmap,
                    std::vector<std::pair<ngpt::Satellite, std::vector<double>>>
                        &satobs) noexcept;

  /// @brief Resolve line(s) of type "SYS / # / OBS TYPES" as RINEX v3.04
  int __resolve_obstypes_304__(const char *) noexcept;

  /// @brief Given a GnssObservable collect info on how to read and formulate it
  ///        (aka observable index column and coefficient)
  vecof_idpair obs_getter(const GnssObservable &obs, SATELLITE_SYSTEM &sys,
  ...
  std::map<SATELLITE_SYSTEM, std::vector<ObservationCode>> __obstmap;
