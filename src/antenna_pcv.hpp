...
public:
  /// @brief Constructor
  AntennaPco(const ObservationCode &obs, SATELLITE_SYSTEM sys, double n = 0e0,
             double e = 0e0, double u = 0e0) noexcept
      : __otype(obs), __ssys(sys), __dn(n), __de(e), __du(u) {}

#ifdef DEBUG
  void dummy_print(std::ostream &) const;
#endif
...
public:
  ...
  explicit AntennaPcoList(const AntennaPco &pco) noexcept : __pco{pco} {}

  std::vector<AntennaPco> &__vecref__() noexcept { return __pco; }
