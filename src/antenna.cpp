...
void ReceiverAntenna::initialize() noexcept {
  constexpr std::size_t mpr_sz =
...
void ReceiverAntenna::set_none_radome() noexcept {
...
bool ReceiverAntenna::radome_is_empty() const noexcept {
...
///           will be ignored.
void ReceiverAntenna::copy_from_cstr(const char *c) noexcept {
  // get the size of the input string.
  std::size_t ant_size = std::strlen(c);

  // max chars to copy would be:
  constexpr std::size_t max_sz =
      antenna_model_max_chars + antenna_radome_max_chars + 1;
...
bool ReceiverAntenna::is_same(const ReceiverAntenna &rhs) const noexcept {
...
///                 model and radome.
bool ReceiverAntenna::compare_model(const ReceiverAntenna &rhs) const noexcept {
...
bool ReceiverAntenna::compare_serial(const char *c) const noexcept {
...
///			         or false otherwise.
bool ReceiverAntenna::has_serial() const noexcept {
...
void ReceiverAntenna::set_serial_nr(const char *c) noexcept {
  constexpr std::size_t start_idx =
      antenna_model_max_chars + antenna_radome_max_chars + 1;
  std::memset(__name + start_idx, ' ', antenna_serial_max_chars + 1);
  std::memcpy(__name + start_idx, c,
              sizeof(char) *
...
void SatelliteAntenna::copy_from_cstr(const char *c) noexcept {
...
bool SatelliteAntenna::is_same(const char *c) const noexcept {
...
void SatelliteAntenna::initialize() noexcept {
