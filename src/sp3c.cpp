...
void Sp3c::rewind(pos_type pos) noexcept {
  if (pos == pos_type(-1)) {
...
///          0: ok
///         >0: error
int Sp3c::get_next_epoch(ngpt::datetime<ngpt::microseconds> &t,
                         std::vector<Sp3EpochSvRecord> &vec,
                         int &sats_read) noexcept {
  ...
  do {
    ...
    if (c != '*') {
      ...
      if (*line == 'P') { /* position and clock line */
        if ((j = get_next_position(line, sys, prn, state, flag)))
          return j + 20;
...
int Sp3c::get_next_position(char *line, ngpt::SATELLITE_SYSTEM &s, int &prn,
                            std::array<double, 4> &state,
  ...
  if (std::any_of(state.begin(), state.begin() + 3,
                  [](double d) { return d == SP3_MISSING_POS_VALUE; })) {
    flag.set(Sp3Event::bad_abscent_position);
  }
  if (state[3] >= SP3_MISSING_CLK_VALUE)
