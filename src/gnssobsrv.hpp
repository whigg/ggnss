...
public:
  explicit GnssRawObservable(ngpt::SATELLITE_SYSTEM sys,
                             ngpt::ObservationCode code) noexcept
      : __sys(sys), __code(code){};

  ngpt::SATELLITE_SYSTEM &satsys() noexcept { return __sys; }

  ngpt::SATELLITE_SYSTEM satsys() const noexcept { return __sys; }

  int band() const noexcept { return __code.band(); }

  ngpt::ObservationCode code() const noexcept { return __code; }

  bool operator==(const GnssRawObservable &o) const noexcept {
  ...
  // 0<=N<2) Notice the parenthesis at the return statement; this function
...
struct __ObsPart {
  ...
  __ObsPart(ngpt::SATELLITE_SYSTEM sys, ngpt::ObservationCode code,
            double c = 1e0) noexcept
      : __type(GnssRawObservable{sys, code}), __coef(c){};

  bool operator==(const __ObsPart &o) const noexcept {
    return __type == o.__type && __coef == o.__coef;
  }

  bool operator!=(const __ObsPart &o) const noexcept { return !(*this == o); }

  /// nominal frequency multiplied by coefficient in MHz
  double frequency() const;

  /// return the type
  GnssRawObservable type() const noexcept { return __type; }
...
public:
  GnssObservable(ngpt::SATELLITE_SYSTEM sys, ngpt::ObservationCode code,
                 double coef = 1e0) noexcept {
    __vec.emplace_back(sys, code, coef);
  }

  GnssObservable(GnssRawObservable obs, double coef = 1e0) noexcept {
    __vec.emplace_back(obs, coef);
  }

  void add(GnssRawObservable obs, double coef = 1e0) noexcept {
    __vec.emplace_back(obs, coef);
  }

  void add(ngpt::SATELLITE_SYSTEM sys, ngpt::ObservationCode code,
           double coef = 1e0) noexcept {
    __vec.emplace_back(sys, code, coef);
  }
  ...
  double frequency() const noexcept {
    ...
    for (const auto &v : __vec)
      frequency += v.frequency();
    return frequency;
  }

  std::vector<__ObsPart> underlying_vector() const noexcept { return __vec; }

  std::vector<__ObsPart> &underlying_vector() noexcept { return __vec; }

  bool operator==(const GnssObservable &) const noexcept;

  bool operator!=(const GnssObservable &a) const noexcept {
    return !(*this == a);
  }

  bool is_of_mixed_satsys() const noexcept {
    if (!__vec.size())
      return false;
    auto sys = __vec[0].__type.satsys();
    for (const auto &obspart : __vec)
      if (obspart.__type.satsys() != sys)
        return true;
    return false;
  }
...
template <size_t N> struct tuple_element<N, ngpt::GnssRawObservable> {
